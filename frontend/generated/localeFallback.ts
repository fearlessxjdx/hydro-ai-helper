...
export const LOCALE_FALLBACK: Record<'zh' | 'en', Record<string, string>> = {
  "zh": {
    ...
    "ai_helper_admin_scenario_testdata_generation_desc": "教师在题目文件页根据题面生成测试数据与函数题模板。",
    "ai_helper_admin_scenario_testdata_strong_model_warning": "⚠️ 正确性优先：请为此场景指定当前可用的最强模型。弱模型写出的生成器与标程常自相矛盾，会反复卡在沙箱验证失败。",
    ...
    "ai_helper_testdata_strong_model_notice": "⚠️ 本功能对模型能力非常敏感：请在 AI 助手配置 →「场景模型」中为「测试数据生成」指定当前可用的最强模型。弱模型写出的生成器与标程常自相矛盾，会反复卡在沙箱验证失败。",
  ...
  "en": {
    ...
    "ai_helper_admin_scenario_testdata_generation_desc": "Teachers generate test data and function-problem templates from the problem statement on the problem files page.",
    "ai_helper_admin_scenario_testdata_strong_model_warning": "⚠️ Correctness first: assign the strongest model you have to this scenario. Weaker models often produce a generator and reference solution that contradict each other and repeatedly fail sandbox verification.",
    ...
    "ai_helper_testdata_badge_executed": "Sandbox-verified",
    ...
    "ai_helper_testdata_strong_model_notice": "⚠️ This feature is highly sensitive to model capability: assign the strongest model you have to the Test Data Generation scenario in AI Helper settings → Scenario Models. Weaker models often produce a generator and reference solution that contradict each other and repeatedly fail sandbox verification.",

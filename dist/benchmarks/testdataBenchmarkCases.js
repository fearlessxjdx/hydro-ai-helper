...
exports.TESTDATA_HARD_BENCHMARK_CASES = void 0;
exports.selectTestdataBenchmarkCases = selectTestdataBenchmarkCases;
 ...
 */
exports.TESTDATA_HARD_BENCHMARK_CASES = [
    {
...
function selectTestdataBenchmarkCases(ids = []) {
    if (ids.length === 0)
        return [...exports.TESTDATA_HARD_BENCHMARK_CASES];
    const requested = new Set(ids);
    const selected = exports.TESTDATA_HARD_BENCHMARK_CASES.filter(item => requested.has(item.id));
    const missing = ids.filter(id => !selected.some(item => item.id === id));
    if (missing.length > 0)
        throw new Error(`未知基准题目：${missing.join('、')}`);
    return [...selected];
}
//# sourceMappingURL=testdataBenchmarkCases.js.map
...
export const LOCALE_FALLBACK: Record<'zh' | 'en', Record<string, string>> = {
  "zh": {
    ...
    "ai_helper_admin_scenario_testdata_generation_desc": "教师在题目文件页根据题面生成测试数据与函数题模板。正确性优先，强烈建议选用最强的模型。",
    ...
    "ai_helper_testdata_review_warning": "测试数据由 AI 生成，输出可能有误。请核对各测试点的 .out 是否正确（可用 std.py 本地验证），写入后建议提交标程并重测整题。",
    "ai_helper_testdata_scale_label": "数据规模",
    "ai_helper_testdata_scale_large": "大（接近约束上限）",
    "ai_helper_testdata_scale_large_warning": "大规模数据的输出由 AI 推演，仅在数据有规律（可由公式推出答案）时可靠。写入后务必用标程提交重测验证；重要考试数据建议选择中小规模。",
    "ai_helper_testdata_scale_medium": "中（约束内中等量级）",
    "ai_helper_testdata_scale_small": "小（人工可验算，默认）",
  ...
  "en": {
    ...
    "ai_helper_admin_scenario_testdata_generation_desc": "Teachers generate test data and function-problem templates from the problem statement on the problem files page. Correctness first — use your strongest model.",
    ...
    "ai_helper_testdata_review_warning": "This data is AI-generated and outputs may be wrong. Verify each .out (you can check locally with std.py); after writing, submit the reference solution and rejudge the problem.",
    "ai_helper_testdata_scale_label": "Data scale",
    "ai_helper_testdata_scale_large": "Large (near constraint limits)",
    "ai_helper_testdata_scale_large_warning": "Outputs for large data are AI-derived and only reliable for patterned data whose answer follows from a formula. Rejudge with the reference solution after writing; prefer small/medium for high-stakes data.",
    "ai_helper_testdata_scale_medium": "Medium (mid-range within constraints)",
    "ai_helper_testdata_scale_small": "Small (hand-checkable, default)",

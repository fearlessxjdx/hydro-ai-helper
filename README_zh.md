...
- **AI 生成测试数据（Beta）** — 在题目文件页（`/p/:pid/files`）根据 Markdown 题面一键生成测试数据
  - 支持传统题与 LeetCode 风格函数题（含类/列表实现的链表题），自动生成 `template.py` / `template.java` / `template.cc` 与 `compile.sh`
  - 支持填空题（完善代码）：自动识别题面中的待完善代码（可手动指定），标程与测试点输出严格匹配题面代码的打印格式
  - 可粘贴已有标准答案：提供后所有测试点输出以该代码为唯一权威推演，并作为 std 文件一并写入
  - 数据规模三档可调（小/中/大）；测试点强制覆盖题面示例与边界组（最小规模、规模上限、特殊值、特殊结构），每个测试点标注设计意图
  - `config.yaml` 评测配置（`user_extra_files` / `subtasks` / `langs`）由插件确定性生成，写入后评测设置自动同步
  - 附带参考标程；所有文件先预览、可编辑、勾选后才写入，覆盖已有文件前明确提示
  - 权限与 Hydro 题目文件管理一致（题目所有者或拥有题目编辑权限），学生无法访问
...
- 场景模型分配：学生对话/学习总结/教学分析/测试数据生成可分别指定专属模型，未配置则跟随全局
...
**AI 生成测试数据（题目文件页，全新）**
- 根据 Markdown 题面一键生成测试点、函数题评测模板（template.py/java/cc + compile.sh）、config.yaml（写入后评测设置自动同步）与参考标程
- 支持传统题、LeetCode 风格函数题（含 class Solution 类方法、类/列表实现的链表题）与填空题（完善代码）
- 可粘贴已有标准答案作为输出的唯一权威；数据规模小/中/大三档，测试点强制覆盖题面示例与边界组
- 骨架模式（AI 故障降级）：不调用 AI 产出结构性文件与空白测试点
- 全量预览、可编辑、勾选写入、覆盖提示；权限与 Hydro 题目文件管理一致
- 新增「测试数据生成」AI 场景（可单独指定模型链）；该场景不限输出长度、单次超时 10 分钟
- AI 输出采用分节文本契约（代码零转义）；面板加载失败时显示可操作错误提示
